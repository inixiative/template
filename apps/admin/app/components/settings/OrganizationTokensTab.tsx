import { Button, Card, CardContent, CardHeader, CardTitle, CreateTokenModal, Table } from '@template/ui/components';
...
export const OrganizationTokensTab = ({ organizationId }: { organizationId: string }) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      label: '',
      render: (token: Token) => {
        const tokenRecord = { ...token, userId: token.userId || auth.user?.id, organizationId };
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0">
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">Create and manage API tokens for this organization</p>
            </div>
