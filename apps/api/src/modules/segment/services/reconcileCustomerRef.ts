...
import { SegmentMemberSource, type SegmentOwnerModel } from '@template/db/generated/client/enums';
...
const providerOf = (customerRef: CustomerRef): { ownerModel: SegmentOwnerModel; ownerId: string } | null => {
  const ownerModel = customerRef.providerModel as unknown as SegmentOwnerModel;
  const fk = customerRefProviderFk(ownerModel);
  const ownerId = fk ? ((customerRef as unknown as Record<string, unknown>)[fk] as string | null) : null;
  return ownerId ? { ownerModel, ownerId } : null;
};

export const dynamicSegmentsOf = (ownerModel: SegmentOwnerModel, ownerId: string, db: Db = defaultDb) =>
  db.segment.findMany({
...
const recordDecision = (row: HydratedCustomerRef, segment: Segment, matches: boolean): void => {
  ...
  if (matches && index === -1) row.segmentMembers.push({ source: SegmentMemberSource.rule, segment });
  if (!matches && index !== -1 && row.segmentMembers[index]!.source === SegmentMemberSource.rule) {
    row.segmentMembers.splice(index, 1);
  }
};
...
export const reconcileCustomerRef = async (
  ...
  const segments = (await dynamicSegmentsOf(provider.ownerModel, provider.ownerId, db)).filter(isReconcilable);
