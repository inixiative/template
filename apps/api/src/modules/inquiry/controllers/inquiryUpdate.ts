...
export const inquiryUpdateController = makeController(inquiryUpdateRoute, async (c, respond) => {
  ...
  if (inquiry.status !== 'draft') throw makeError({ status: 400, message: 'Only draft inquiries can be updated', requestId: c.get('requestId') });

  const isSource = await checkIsSource(db, inquiry, user.id);
  if (!isSource) throw makeError({ status: 403, message: 'Only the source can update', requestId: c.get('requestId') });
