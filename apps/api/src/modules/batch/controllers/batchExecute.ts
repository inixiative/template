...
export const batchExecuteController = makeController(batchExecuteRoute, async (c, respond) => {
  ...
  if (batchId) throw makeError({ status: 400, message: 'Batch requests cannot be nested' });
  ...
  if (maxPerRound > 50) {
    throw makeError({ status: 400, message: 'Maximum 50 requests per round' });
  }

  const totalRequests = body.requests.reduce((sum, round) => sum + round.length, 0);
  if (totalRequests > 100) {
    throw makeError({ status: 400, message: 'Maximum 100 total requests' });
  }
