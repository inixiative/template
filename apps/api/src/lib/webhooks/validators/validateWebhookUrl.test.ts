...
describe('validateWebhookUrl', () => {
  describe('enforced', () => {
    ...
    it('rejects malformed URLs', () => {
      expect(() => validateWebhookUrl('not a url', enforce)).toThrow('must be a valid URL');
    });
    ...
    it('allows public literal addresses', () => {
      expect(() => validateWebhookUrl('https://93.184.216.34/hooks', enforce)).not.toThrow();
      expect(() => validateWebhookUrl('https://[2606:2800:220:1::1]/hooks', enforce)).not.toThrow();
    });
  });

  describe('not enforced (local)', () => {
    const relaxed = { enforce: false };

    it('allows http and private hosts', () => {
      expect(() => validateWebhookUrl('http://localhost:3000/hooks', relaxed)).not.toThrow();
      expect(() => validateWebhookUrl('http://10.0.0.1/hooks', relaxed)).not.toThrow();
    });
