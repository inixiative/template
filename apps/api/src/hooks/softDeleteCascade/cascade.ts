 ...
 * @uses none
 */
import { type HookDb, type ModelName, type RuntimeDelegate, toAccessor } from '@template/db';
...
const delegateFor = (hookDb: HookDb, model: string): RuntimeDelegate =>
  hookDb[toAccessor(model as ModelName)] as unknown as RuntimeDelegate;

export const tombstoneChildren = async (hookDb: HookDb, model: string, row: Row) => {
  for (const child of childRelations(model)) {
    if (HARD_DELETE_ON_TOMBSTONE[model]?.includes(child.model)) {
      await delegateFor(hookDb, child.model).deleteMany({ where: fkWhere(child, row) });
    } else if (child.hasDeletedAt) {
      await delegateFor(hookDb, child.model).updateManyAndReturn({
...
export const reviveChildren = async (hookDb: HookDb, model: string, row: Row, priorDeletedAt: unknown) => {
  for (const child of childRelations(model)) {
