...
- `sample: { percent, from? }` is not in the variant API: a variant's audience is exactly `segmentId` or `internalSegment`. Deterministic sampling over the `CustomerRef` id follows separately.
