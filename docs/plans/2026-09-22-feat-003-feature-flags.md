...
- Sampling is `FeatureFlagVariant.sample: { from, to }`, applied in the resolver fold over the `CustomerRef` id's random tail at `FeatureFlag.sampleOffset` (a random column default, never in the API), not a stored id list, not a segment property and not a rules-engine operator (architect: the id is already a hex number; one salt lives on the flag; segments without flags have no sampling). No reconcile is involved; a sampled arm has no member rows. `sampleCustomerRefIds` and the ticket's `sample: { percent, from? }` are gone.
