...
- Flag, variant and `Segment.name` uniques are partial on `deletedAt IS NULL` so a deleted slug, label or segment name can be reused; the `Segment.name` uniques also exclude `featureFlagInternal` rows, so an internal segment named `${flag.slug}/${variant.label}` never collides with a shared name or with another flag's variant of the same label, and re-pointing never needs a unique name. The segment change is a FEAT-021 model change made here.
...
- Sampling is `Segment.sample: { from, to, offset }`, a post-filter over the `CustomerRef` id's random tail applied after rule evaluation on both rails, not a stored id list and not a rules-engine operator (architect's ruling: the id is already a hex number, read three digits at an offset from the end and ask `between`). The API takes `{ from, to }` only; `offset` is internal — `FeatureFlag.sampleOffset` is a random column default and copied to each variant's internal segment (arms disjoint, flags independent); a shared segment draws its own at first set and keeps it. `sampleCustomerRefIds` and the ticket's `sample: { percent, from? }` are gone.
- `slug` and `subjectModel` immutability on `FeatureFlag` (this plan's addition).
- An internal segment is `Segment.featureFlagInternal Boolean` (architect's ruling), not a back-pointer FK to its variant; the variant's `segmentId` is the only edge between the two, so the schema has no mutual-FK pair. The soft-delete cascade registry walks parent→child FKs and cannot reach the segment from the variant, so the `featureFlagVariant` hook tombstones and revives the internal segment with the variant's own timestamp. `/// @permissions(hydrate: false)` stays on `FeatureFlag.segment`, `FeatureFlagVariant.segment` and `SegmentMember.customerRef` (permissions never flow through those edges), and `hydrate()` throws on a cycle instead of walking it.
