...
- **ALL worktree lifecycles go through the repo scripts** — `bun run worktree:create <base> <new-branch>` (or `worktree:create <existing-branch>` to attach; `--skip-db` for docs-only work that needs no databases or Docker), `bun run worktree:destroy <name>`, `bun run worktree:list`. NEVER `git worktree add`/`remove` by hand: the scripts provision and tear down the whole slot environment (Docker pre-flight, ports, `.env.local`/`.env.test` synced against the branch's examples, per-slot databases and buckets, dependency install, generated artifacts, schema push) — a hand-rolled worktree is a broken half-environment plus orphaned state. Create fails fast when Docker/Postgres is down and ends green **ready** only when every artifact and push landed; yellow **WITH GAPS** lists what to rerun. If all slots are in use, ask which worktree to destroy; verify it is merged and clean first. The `bashMainCheckoutGuard` hook (`.claude/hooks/`) denies a bare mutating command (`bun add/remove/update`, `git commit/push/merge/rebase/cherry-pick`, `git worktree add/remove/move/prune`, `prisma migrate dev`, `bun run db:migrate`) from the main checkout once this session has worked in a worktree — name the directory with `cd <worktree> && …`. See `.claude/skills/worktrees/SKILL.md`.
