...
export const Prerequisites: React.FC<PrerequisitesProps> = ({ onComplete }) => {
	...
	const allDone = useMemo(
		() =>
			...
			pscaleCLI.status !== 'pending' && pscaleCLI.status !== 'checking' &&
			vercelCLI.status !== 'pending' && vercelCLI.status !== 'checking' &&
			railwayCLI.status !== 'pending' && railwayCLI.status !== 'checking' &&
	...
	const renderCheck = (check: Check, name: string) => {
		...
		if (check.status === 'checking') return <Text color="cyan"><Spinner type="dots" /> {name}</Text>;
		...
		return (
			<Text color={color}>
				{icon} {name}{check.version ? ` (${check.version})` : ''}
			</Text>
		);
	};
	...
	return (
		<Box flexDirection="column" padding={1}>
			<Box marginBottom={1}>
				<Text bold underline>Prerequisites Check</Text>
			</Box>
			...
			{allDone && allPassed && (
				<Box marginTop={1}>
					<Text color="green" bold>✓ All prerequisites met!</Text>
				</Box>
			)}
		</Box>
	);
