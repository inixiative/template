...
const getProgressDisplay = (config: ProjectConfig): Array<{ label: string; completed: boolean }> => {
	...
	return [
		...
		// Superadmin app
		{
			label: superadminProjectId ? `Superadmin: Project created (${superadminProjectId})` : 'Superadmin: Project created',
...
export const VercelSetupView: React.FC<VercelSetupViewProps> = ({ onComplete, onCancel }) => {
			...
			<Box flexDirection="column" padding={1}>
				<Box borderStyle="round" borderColor="cyan" paddingX={2} paddingY={1}>
					<Box flexDirection="column">
						<Text bold color="cyan">GitHub Integration Required</Text>
						...
						<Box flexDirection="column" marginTop={1} marginLeft={2}>
							<Text>1. Visit: <Text color="cyan">https://github.com/apps/vercel</Text></Text>
							<Text>2. Click "Configure" (or "Install" if not installed)</Text>
							<Text>3. Select organization: <Text color="yellow">{config?.project.organization}</Text></Text>
							<Text>4. Grant access to repository: <Text color="yellow">{config?.project.name}</Text></Text>
	...
	return (
		<Box flexDirection="column" padding={1}>
			...
			<Box marginTop={1}>
				{setupState === 'new' && (
					<Text color="cyan">⚡ Ready to provision Vercel projects</Text>
				)}
				{setupState === 'stale' && (
				...
				{setupState === 'complete' && (
					<Text color="green">✓ Vercel setup complete</Text>
				)}
			</Box>
