 ...
 */
const listVercelConnections = async (
	infisicalProjectId: string
 ...
 */
export const listVercelSyncs = async (
	infisicalProjectId: string
): Promise<VercelSecretSync[]> => {
	const infisicalToken = getInfisicalToken();
...
export const createVercelConnection = async (
	...
	const response = await fetch(
		'https://app.infisical.com/api/v1/app-connections/vercel',
		{
			method: 'POST',
			headers: {
				'Authorization': `Bearer ${infisicalToken}`,
...
export const createVercelSync = async (
	...
	const response = await fetch(
		'https://app.infisical.com/api/v1/secret-syncs/vercel',
		{
			method: 'POST',
			headers: {
				'Authorization': `Bearer ${infisicalToken}`,
