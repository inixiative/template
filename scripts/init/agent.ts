...
import { updateConfigField, setProgressComplete, isProgressComplete } from './utils/configHelpers';
...
export async function initializeProject(config: InitConfig): Promise<InitResult> {
	try {
		...
		const infisicalResult = await setupInfisical(
			config.infisicalOrgId,
			async () => {
		...
		const planetscaleResult = await setupPlanetScale(
		...
		const railwayResult = await setupRailway(
			config.railwayWorkspaceId,
			async () => {
