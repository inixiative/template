 ...
 */
const initPrismaMigrationTable = async (
...
export const setupPlanetScale = async (
	...
	try {
		...
		let region = config.planetscale.region;
		let database = config.planetscale.database;
		let tokenId = config.planetscale.tokenId;
	...
	// Suppressed for TUI: console.log(`  Organization: ${organization}`);
		...
		if (!(await isProgressComplete('planetscale', 'createStagingBranch'))) {
			try {
				stagingBranch = await retryWithTimeout(
					() => createBranch(organization, databaseName, 'staging', 'prod'),
					{
						maxRetries: 100,
						delayMs: 3000,
						retryCondition: (error) => {
