...
import { createProject, linkGitHub, addEnvVar, checkGitHubIntegration, updateProjectSettings, createCustomEnvironment } from '../api/vercel';
import { isAppInstalled } from '../api/github';
import { markComplete, isComplete, setError } from '../utils/progressTracking';
 ...
 */
const getOrCreateVercelConnection = async (
...
export const setupVercel = async (
	...
	try {
		...
		if (!(await isComplete('vercel', 'promptedForGithub'))) {
			...
			const isVercelAppInstalled = await isAppInstalled(
