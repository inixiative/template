...
describe('cache size guard', () => {
  ...
  it('returns the value but does not store it when the payload is oversized', async () => {
    const key = cacheKey('user', 'size-guard-oversized');
    let runs = 0;
    const produce = async () => {
      runs += 1;
      return oversized();
    ...
    expect(await cache(key, produce, 60)).toBe(oversized());
    expect(await getRedisClient().get(key)).toBeNull();

    await cache(key, produce, 60);
    expect(runs).toBe(2);
  });

  it('still caches a value under the ceiling', async () => {
    const key = cacheKey('user', 'size-guard-small');
    let runs = 0;
    const produce = async () => {
      runs += 1;
      return 'small';
    ...
    expect(runs).toBe(1);
    expect(await getRedisClient().get(key)).not.toBeNull();
  });

  it('upsertCache reports failure and writes nothing when the payload is oversized', async () => {
    const key = cacheKey('user', 'size-guard-upsert');

    expect(await upsertCache(key, oversized())).toBe(false);
    expect(await getRedisClient().get(key)).toBeNull();
  });
});
