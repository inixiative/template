...
describe('mutationLifeCycle', () => {
  describe('registerDbHook', () => {
    it('registers a hook for a specific model and action', async () => {
      ...
      registerDbHook('test-hook-1', 'User', HookTiming.before, [DbAction.create], hookFn);
    ...
    it('registers a global hook using wildcard model', async () => {
      ...
      registerDbHook('test-hook-3', '*', HookTiming.before, [DbAction.create], hookFn);
  ...
  describe('executeHooks', () => {
    ...
    it('passes hook options including result for after hooks', async () => {
      ...
      registerDbHook('test-hook-with-result', 'User', HookTiming.after, [DbAction.create], hookFn);
  ...
  describe('DbAction enum', () => {
    it('has all expected actions', () => {
      expect(DbAction.create as string).toBe('create');
      expect(DbAction.createManyAndReturn as string).toBe('createManyAndReturn');
      expect(DbAction.update as string).toBe('update');
      expect(DbAction.updateManyAndReturn as string).toBe('updateManyAndReturn');
      expect(DbAction.delete as string).toBe('delete');
      expect(DbAction.deleteMany as string).toBe('deleteMany');
      expect(DbAction.upsert as string).toBe('upsert');
    });
  });
