import type { ContactTypeDef } from '@template/shared/contact/defs/base';
import { z } from 'zod';

// WeChat: identifier is a WeChat ID; no canonical public profile URL.
export type WechatValue = { handle: string };

const stored = z.object({ handle: z.string().min(1) });

export const wechatDef: ContactTypeDef<WechatValue, WechatValue> = {
  inputSchema: stored,
  parseInput: (v) => v,
  valueSchema: stored,
  toValueKey: (v) => v.handle.toLowerCase(),
  subtype: { mode: 'forbidden' },
  uniqueness: 'per-owner',
  display: { label: 'WeChat', icon: 'MessageCircle' },
};
