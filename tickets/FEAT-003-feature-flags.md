...
- **Percentage rollout is sortition, and sortition is a segment.** A `CustomerRef` id is a uuidv7 whose last 15 hex digits are random, so it is already the coin flip: `Segment.sample: { from, to, offset }` reads three of those digits starting at digit position `offset` from the tail as a point on 0–100 and keeps the customers whose point sits in `[from, to)`. It is a post-filter after rule evaluation on both rails, not a stored id list and not a rules-engine operator, so it is deterministic, recalculates with the rule, self-enrolls new customers, and widening keeps everyone already in. A variant's internal segment takes `sample: { from, to }` and its offset from `FeatureFlag.sampleOffset`, random on create, so one flag's arms never overlap and different flags read different digits (thirteen offsets before two flags share one).
