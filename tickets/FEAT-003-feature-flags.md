...
- **Inline segments are owned by the variant and hidden — and the hiding is enforced at the save gate, not only in the picker.** An audience nobody has named yet ("orgs in segment S on plan X", "a random 20%") has to be named by somebody; the only question is where it lives. A segment defined from a variant keeps its provider ownership (`ownerModel` + FK, as any segment) and additionally carries `Segment.featureFlagVariantId`: it is deleted with the variant, edited only through it, and excluded from the owner's segment list and every picker. Because the API also takes raw JSON, exclusion is enforced where references are written: the `segmentConditions` save gate refuses a rule that names an inline segment (alongside its existing "not live / not owned" refusals), and the flag / variant hooks refuse a `segmentId` that is inline for any variant but their own. Otherwise deleting a flag could hollow out another audience. Explicit ownership, not a `hidden` boolean: the lifecycle is what made hidden segments feel wrong, and it is pinned here. The segment list shows what people named on purpose; a variant may also point at one of those.
- **Percentage rollout is sortition, and sortition is a segment.** *Manual* sortition needs nothing new: "roll out to 20%" samples and writes an inline static segment whose rule is `id in [...]` (FEAT-021's hand-picked-audience convention). The sample's population is either all of the owner's customers of the subject kind or the members of a named segment — "this segment, then 30% of it" composes as `sample: { percent, from: segmentId }` — so segment targeting and percentage selection still compose without a weight column. Static means the enrollment is fixed until changed; raising to 30% samples again from the remainder into the same list. *Automatic* sortition — a deterministic bucket over the ref id so new customers enroll themselves — is one json-rules operator (`bucket`: hash of `id` + a seed, modulo 100, compared to a threshold) that must compile identically on both rails (`toPrisma` SQL and `check()` JS agreeing on the hash); with it, "in segment S and in bucket" is one dynamic segment rule. That is primitive work, ratified separately; the flag design does not change when it lands, a rollout variant just points at a dynamic segment instead of a static one. The two behave differently — fixed enrollment versus self-enrolling — and that is a choice the owner makes per variant, not a resolver mode.
...
- **Subject kind is explicit.** `FeatureFlag.subjectModel` is `CustomerModel` (User | Organization | Space): the kind of customer the flag addresses. It is enforced at resolution — members of a variant's segment whose `customerModel` differs are ignored — because a segment has no customer-kind column and its rule may mix kinds. An inline segment is written with the subject kind in its rule.
...
model FeatureFlag {
  ...
  // Audience gate — optional; a segment owned by the same provider (shared or inline).
...
// On Segment (FEAT-021):
//   featureFlagVariantId String? @unique @db.VarChar(36)   — set ⇒ inline: owned by, deleted with,
//   edited only through the variant; excluded from list routes and pickers.
...
- Routes follow `Segment`'s per-owner shape: `featureFlags` create + read-many on `/me`, `/organization/:id`, `/space/:id`, and `/admin` for `platform`; `/featureFlag/:id` read / update / delete; `/featureFlag/:id/variants` list + create; `/featureFlagVariant/:id` update / delete; `meReadManyFeatureFlags` (subject-facing, values only). Reordering is a plain update of `position` on the variant; the ordered-list hook does the shifting. A variant create / update takes either `segmentId` (a shared segment of the owner) or `inlineSegment: { type, conditions }`; the service writes the inline row with `featureFlagVariantId` set. A `sample: { percent, from?: segmentId }` affordance on the variant writes the inline static id-list segment described above.
- Segment list routes and the picker source exclude rows with `featureFlagVariantId`; `/segment/:id` update / delete refuse them (edit through the variant). `estimateSegmentReach` gets its route here, since a variant row is where reach is displayed.
...
- Owner-context flag pages (org / space / me) list the owner's flags and their variants with reach per row; a variant's segment is picked from the owner's segments or defined inline. Inline *rule* authoring is JSON through the API until the segment builder exists (see Open); sampling and hand-picked lists need no builder.
...
- **`Segment.featureFlagVariantId`** with the exclusion in list routes / pickers, delete cascade from the variant, and edit-through-variant refusal. Small, and it is the whole "hidden segment" mechanism.
...
- **Segment builder.** `SegmentsPage` is a read-only table and rules-builder is wired only for the email surface; `segmentLensFor(owner)` exists server-side. The builder is the INFRA-017 / INFRA-024 consumer FEAT-021 already names. Once it ships, inline rule authoring on a variant is the builder embedded in the variant form.
- **Inline segment volume.** Every rollout and every inline rule is a `Segment` plus its `SegmentMember` rows and a place in the reconcile sweep. That is the price of one materializer, and it is the right price; the first thing to measure is sweep time as flag-owned dynamic segments accumulate, and whether disabled flags' inline segments should be frozen (flipped static) rather than reconciled.
