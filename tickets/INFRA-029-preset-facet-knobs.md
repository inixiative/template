...
**Status**: 🚧 In Progress — rules-builder **0.26.1 shipped 2026-08-27** (adversarial pass on 0.26.0, 21 agents: a bare `condition: { field … }` template lost its card on the first tune — templates/slots/ids/recognition now read the builder's own shape via `normalizeGroups`; `FacetCondition` types `Facet.condition` so templates need no cast; a matched preset is settled before path facets; crossing-slot presets are an `ambiguous` violation; option labels come from the slot's own control). 0.26.0 was (`4a06d89` main; npm + `v0.26.0` dist tag on inixiative and the userevidence fork; `test/decoration.variables.test.ts`, 16 tests): `Variable`, `presetSeed`, `variableSlots`, `VariableControl` on atomic nodes, ranked preset recognition, aggregate presets, validator rules — mechanics 1–5 below as written. Remaining: Zealot decoration move — ticketed ZLT-4507 (pin bump to v0.26.1 landed as its first commit)
**Assignee**: Aron
**Priority**: Medium (Zealot carries a side-channel that covers it today; the side-channel is the cost)
**Created**: 2026-08-26
**Updated**: 2026-08-27 (0.26.1 shipped)
...
A **variable** is a leaf whose value source is left to the author. It is written as a **sibling key** in the leaf's value-source position — `variable` beside `value` / `path` / `bind`, mutually exclusive with them — never nested inside `value`: a real `value` can be any JSON (a Json column literal, a `DateExpr`, a range pair), so a marker object inside it is impossible to tell from data. The word sits in that same family — `value` (literal), `path` (same-row / context), `bind` (server-supplied at evaluation), `variable` (*author-supplied at authoring time*). "Knob" is the renderer's metaphor for the control, not the template's word for the slot.
...
// The first consumer, as it actually ships in Zealot (segmentDecoration.ts RULE_PRESETS[0]):
{
  ...
  condition: {
    ...
    variable: {},                                              // threshold: OPEN (no value/path/bind) — inserts incomplete, the save gate blocks until filled
    condition: { all: [
      { field: 'createdAt', dateOperator: 'within', variable: { default: { this: 'year' } } },            // window: variable with a default
...
- **Variable = open; default = default.** `variable: {}` is a legal open slot and inserts with *no* value-source key (the tree state a freshly added leaf has — `isLeafComplete` false, `validateRule` blocks save). A default, when present, lives *in* the slot. The threshold above is deliberately open: `default: 0` would turn today's fail-loud incomplete rule into a silently everyone-matching segment.
...
- **Builder-side template, not a `Condition`.** A leaf carrying `variable` and no `value`/`path`/`bind` is not a valid `Condition` (`validateRule` → `missing_value_source`) until filled, so the marker never reaches json-rules and can never be mistaken for data. `validateDecoration` substitutes a present default and validates against the lens; for an absent default it omits the slot key (no synthesised placeholder — `checkRuleAgainstLens` gates only string enum literals, so `''` is a false violation and `0` is checked by nothing).
- **Value-source variables only in 0.26.** An operator slot would need its own sibling spelling (an `operator` is a string, so no collision — but it is a different position), and operator variables are *not* free by construction: the operator selects the value shape (`valueShapeForOperator`), `validateRule` forbids `value` on a `none`-shape operator and demands a pair for `between`, so a variable operator reshapes or deletes its sibling. Deferred, with the constraint stated: an operator slot's options must share one value shape unless the sibling value is also a variable.
...
0. **The builder's shape is the identity (0.26.1).** The array sub-builder reads `condition`/`filter` through `asGroupRoot` and commits the group back, so `X` and `{ all: [X] }` are one rule to the engine and one tree to the builder. Templates, slot paths, `facetId`, and recognition all read that shape (`normalizeGroups`, now in `core`) — a bare and a grouped spelling are one facet, and each recognizes the rules the other saved (Zealot's `daysSinceLastActivity` rows are bare). Found by the adversarial pass on 0.26.0; every template in the 0.26.0 suite was grouped, which is why 16/16 passed.
...
2. **Recognition: fill, compare, rank.** `matchFacet`'s preset arm compares `canonical(fill(template, node))` to `canonical(node)` — at each template leaf carrying `variable`, the node's own value-source key (`value` / `path` / `bind`) replaces it — and accumulates into `best` by **fewest wildcard slots** (the existing `bestLead` idiom) instead of returning the first hit, so a fixed-`100` facet beats a variable one on a `100` rule and array order never decides which card a saved rule wears. Do not add a preset rejection rule — catch-all + stricter projection is sanctioned twice in the file.
...
- Cards: `rewardsRedeemedThisYear` → the facet above ("Rewards Redeemed", threshold open, window variable defaulting to this year — today's `AggregateRow` exposes operator, threshold *and* window; fixing the window would orphan every saved rule on `{ last: 'year' }` / `{ ago: { days: 90 } }`). `daysSinceLastLogin` → a single `lastLoginAt notWithin` / `notAfter` leaf (json-rules 2.19.6–2.19.8), no facet. `daysSinceLastActivity` stays a facet with a window variable, **permanently** — ruling settled 2026-08-31 (Aron): manual activity IS activity. The `lastFanMission` pointer's after-create hook skips `activityType` (manual) rows by design, so a `lastFanMission.createdAt` leaf is a different set (an advocate whose only recent touch is admin-logged activity would flip to "inactive") — the card never migrates to it; the `fanMissions none (createdAt within …)` shape is the correct one, and 0.26.1's bare-condition normalization covers its authored spelling. Zealot cutover ticketed as ZLT-4507.
