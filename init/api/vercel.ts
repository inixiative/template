 ...
 */
export const createProject = async (projectName: string, teamId?: string): Promise<VercelProject> => {
  try {
 ...
 */
export const getProject = async (projectName: string, teamId?: string): Promise<VercelProject | null> => {
  try {
    const teamFlag = teamId ? `--scope ${teamId}` : '';
